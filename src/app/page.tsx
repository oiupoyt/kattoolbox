            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
              {featuredTools.map((t) => (
                <Link
                  key={t.slug}
                  href={`/tools/${t.slug}`}
                  className="group block border border-[#1c1c1c] bg-[#060606] hover:border-blue-900 hover:bg-[#0c0c0c] p-3 transition-colors"
                >
                  <div className="text-xs font-medium text-gray-200 group-hover:text-blue-400 transition-colors">
                    {t.name}
                  </div>
                  <p className="mt-1 text-[11px] text-[#555] line-clamp-2 leading-snug">
                    {t.description}
                  </p>
                </Link>
              ))}
